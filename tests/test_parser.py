...
def test_parse_index_returns_sorted_unique_seasons(index_html):
    """parse_index should return each season once, sorted, with absolute HTTPS URLs."""
...
def test_parse_season_extracts_all_puzzles(season_html):
    """parse_season should find every puzzle row and skip header/legend rows."""
    puzzles = parse_season(season_html, season=42, source_url="http://x/42")
    # 10 puzzles in episode 8011 + 10 in episode 8012.
...
def test_parse_season_second_episode(season_html):
    """Puzzles in the second episode should have correct episode and date."""
    puzzles = parse_season(season_html, 42, "http://x/42")
    ep2 = [p for p in puzzles if p["episode"] == "8012"]
    assert len(ep2) == 10
    assert all(p["air_date"] == "2024-09-10" for p in ep2)
    # Category with ampersand survives HTML-entity unescaping.
    before_after = next(p for p in puzzles if p["round_name"] == "Round 2" and p["episode"] == "8011")
    assert before_after["category"] == "Before & After"
    assert before_after["solution"] == "ANIMATED SHORT ATTENTION SPAN"


def test_parse_season_prize_puzzle_type(season_html):
    """R3* rows should have puzzle_type 'Prize Puzzle', not 'Round'."""
    puzzles = parse_season(season_html, 42, "http://x/42")
    prize = [p for p in puzzles if p["puzzle_type"] == "Prize Puzzle"]
    assert len(prize) == 2  # one per episode in the fixture
    assert all(p["round_name"] == "Round 3" for p in prize)
