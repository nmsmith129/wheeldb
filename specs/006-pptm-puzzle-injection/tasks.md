...
- [X] T003 [P] Write FAILING unit tests in `tests/unit/test_storage.py` for `PuzzleStore.puzzles_for_season(season)`: returns the season's rows as `Puzzle` objects (correct attributes + derived `puzzle_type`); returns `[]` for an absent season; covers all three types present. Print the per-type counts on failure (Principle V).
...
- [X] T005 Add `class GameError(WheelDBError)` with a docstring to `src/wheeldb/errors.py` (extends the existing hierarchy — Principle IV). Makes T004/T009 error paths importable.
- [X] T006 Implement `PuzzleStore.puzzles_for_season(season) -> list[Puzzle]` in `src/wheeldb/storage.py` (reuse the existing row→`Puzzle` mapping; no new data path). Pass T003. (Add the parity method to `CsvPuzzleStore` only if/when the CSV source is in scope on this branch.)
- [X] T007 Implement the macro-safe package engine in `src/wheeldb/pptx_inject.py` to pass T004: read all parts from a source `.pptm`, rewrite only specified parts, copy every other entry through byte-for-byte (preserving names/compression, incl. `ppt/vbaProject.bin`), write atomically (temp file then rename), and refuse to overwrite an existing target. Docstring every method (Principle VI).
...
- [X] T012 [US1] Implement `inject_puzzles(template_path, out_path, slot_assignments)` and the `SLOT_MAPPING` table in `src/wheeldb/pptx_inject.py` (build on the T007 engine; resolve each slot via its stable anchor — placeholder token or named shape, never a positional index — and set the solution + category run, XML-escaped; raise `GameError` on a missing anchor). Pass T010.
- [X] T013 [US1] Implement `select_puzzles(...)`, `next_game_number(...)`, and `generate_game(season, *, store, games_dir, seed, template_path)` in `src/wheeldb/gamegen.py`: read via `puzzles_for_season`, group by `puzzle_type`, sample 4/3/1 without replacement using a local `random.Random(seed)`, assign to slots, **create `games_dir` if absent (FR-002)**, pick the next number, and write via `inject_puzzles`; return the created path. Pass T008/T009.
- [X] T014 [US1] Add the `game` subcommand to `src/wheeldb/cli.py` (positional `season`; `--seed`, `--db`, `--games-dir` per contracts/cli.md — no `--format` in this feature); call `generate_game`; print the spoiler-free summary (file + slot counts) to stdout; map `GameError` and a bad argument to a clear stderr message and exit 2. Pass T011.
- [X] T015 [US1] Export `generate_game`, `inject_puzzles`, and `GameError` from `src/wheeldb/__init__.py` (`__all__`) per [contracts/library-api.md](contracts/library-api.md).
...
- [X] T017 [US2] Ensure `src/wheeldb/cli.py` (and `GameError` messages raised in `gamegen.py`) emit only file path, season, and counts — never solution or category text — including the insufficient-puzzles and season-absent errors (Decision 6/7). Each failure mode (template missing, season absent, insufficient type, numbering exhausted, slot unlocatable) MUST produce a distinguishable message (FR-014) and validation MUST run in the fixed order template → season presence → per-type counts → numbering (FR-015), short-circuiting on first failure with no file written (FR-016). Pass T016.
...
- [X] T019 [US3] Write FAILING unit tests in `tests/unit/test_gamegen.py`: a season with fewer than 4 Round / 3 Toss-Up / 1 Bonus puzzles raises `GameError` naming the short type and count and writes no file (all-or-nothing); an absent season raises `GameError`. Print the per-type availability.

### Implementation for User Story 3

- [X] T020 [US3] In `src/wheeldb/gamegen.py`, validate per-type availability **before** writing (raise `GameError` on any shortfall or absent season, in the fixed order from FR-015: template → season presence → per-type counts → numbering) and enforce the slot→type assignment so each slot range receives only its type. Pass T018/T019. (Core selection exists from T013; this pins type-correctness and the all-or-nothing error for every failure mode, per FR-016.)
...
- [X] T021 [P] Update `README.md` with `wheeldb game <season> [--seed] [--db] [--games-dir]` usage, the `games/wof[N].pptm` naming rule, and notes that the template/VBA is preserved and generation is spoiler-free.
- [X] T022 [P] Docstring/lint pass over `src/wheeldb/pptx_inject.py`, `src/wheeldb/gamegen.py`, and the `storage.py`/`cli.py`/`errors.py` additions (Principle VI); ensure no line exceeds the 100-char limit; run `py -m ruff check src tests` if ruff is available, else a manual check.
- [X] T023 Run the full offline suite `py -m pytest -ra` and confirm green (including the existing 001/002 tests — integration across features, Principle III).
- [X] T024 Execute the [quickstart.md](quickstart.md) validation scenarios (1–6) and confirm the Acceptance items.
